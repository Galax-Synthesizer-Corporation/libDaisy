...
class GPIOInterruptHandler
{
  ...
  private:
    ...
    void                   *exti_cb_ctx_[EXTI_NUM_CHANNELS];
};
...
void GPIO::Init()
{
       ...
       || cfg_.mode == Mode::INTERRUPT_BOTH)
    {
        ...
        handler.RegisterPin(cfg_.pin, cfg_.callback, nullptr);
...
{
    GPIOInterruptHandler::instance().RegisterPin(cfg_.pin, cb, ctx);
}
